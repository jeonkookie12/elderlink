...
export default function AdminProfile() {
  ...
  const validateUsername = (username) => {
    ...
    if (username && !usernamePattern.test(username)) {
      setUsernameError(
        "Username must be 8-20 characters, can contain letters, numbers, '.', '_', but cannot start/end with or repeat '.' or '_'"
      );
      return false;
    } else {
      setUsernameError('');
  ...
  const profileImage = previewImage || (user.profile && user.profile.trim() !== ''
    ? `http://localhost/elder-dB/admin-profile/${user.profile}`
    : null);
  ...
  return (
    <div className="font-sans min-h-screen flex items-center justify-center px-4 py-10">
          ...
          <div className="bg-white rounded-lg w-full max-w-md mx-4 p-6 flex flex-col">
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between items-center px-2">
                <span className="text-sm text-gray-500">Expires in: {formatTime(otpTimer)}</span>
                <button
      ...
      <div className="w-full max-w-4xl mx-auto bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-md p-6">
