...
import pwd from "../../src/assets/pwd.jpg";

export async function generatePWDCard(data) {
  ...
  tempDiv.innerHTML += `
    <div class="grid grid-cols-3 items-center text-center">
        <div class="p-6">
            <img src="${sabang}" alt="Barangay Logo" class="w-20 max-w-full h-auto mx-auto" />
        </div>

        <div class="px-2">
            <h2 class="text-base font-semibold">REPUBLIC OF THE PHILIPPINES</h2>
            <h1 class="text-base">PROVINCE OF CAVITE </h1>
            <h1 class="text-base">CITY OF DASMARIÑAS</h1>
            <h2 class="text-base font-medium text-green-800">BARANGAY SABANG</h2>
        </div>

        <div class="flex items-center justify-center space-x-4 p-6">
            <img src="${etivac}" alt="Cavite Logo" class="w-20 h-20 object-contain" />
            <img src="${pwd}" alt="PWD Logo" class="w-20 h-20 object-cover rounded-full" />
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-2 items-start">
      ...
      <div class="flex flex-col w-full md:w-2/3 space-y-4 self-start">
        ...
        <!-- Address and Type of Disability grouped together -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <p class="text-xs font-semibold text-black-600 text-left">Type of Disability</p>
            <p class="text-xl font-medium text-left">${data.type_of_disability || '-'}</p>
          </div>
        </div>
      </div>
    </div>
